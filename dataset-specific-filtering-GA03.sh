...
grep $hash 02-output-CMAP/*.tsv | awk -F $'\t' 'BEGIN {OFS = FS} {print $5,$2,$3,$4,$7,$20,$21}' | sed 's/\t/,/g' > raw/$hash.cmap.csv

awk -F',' ' ($4 < 350 ) ' raw/$hash.cmap.csv > $outfolder/$hash.filtered.cmap.csv #Remove deeper depths for interpolation to work
